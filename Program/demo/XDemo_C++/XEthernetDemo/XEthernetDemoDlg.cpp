...
// XEthernetDemoDlg.cpp : implementation file
//
...
CXEthernetDemoDlg* demo_dlg_;

CXEthernetDemoDlg::CXEthernetDemoDlg(CWnd* pParent /*=NULL*/)
	: CDialog(CXEthernetDemoDlg::IDD, pParent)
	, _frame_counter(0)
	, _line_lost_num(0)
	, _is_non_hilo(0)
	, _en_offboard_correct(0)
	, _en_piecewise_correct(0)
{
...
void CXEthernetDemoDlg::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CXEthernetDemoDlg, CDialog)
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	//}}AFX_MSG_MAP
	ON_NOTIFY(TCN_SELCHANGE, IDC_TAB1, &CXEthernetDemoDlg::OnTcnSelchangeTab1)
...
// CXEthernetDemoDlg message handlers

BOOL CXEthernetDemoDlg::OnInitDialog()
{
	...
	_control_tab.InsertItem(3, "Advance");
	...
	_adv_dlg.Create(IDD_ADV_DIALOG, GetDlgItem(IDC_TAB1));
	...
	_adv_dlg.MoveWindow(&rect);
	_adv_dlg.ShowWindow(FALSE);
	...
	_config_dlg._xoff_correct_  = _xoff_correct_;
	_config_dlg._xsystem_		= &_xsystem;
	_config_dlg._xtransfer_		= &_xtransfer;
	_config_dlg._xpiecewise_correct_ = &_xpiecewise_correct;

	_operate_dlg._xcommand_		= &_xcommand;
	_operate_dlg._xacquisition_ = &_xacquisition;
	_operate_dlg._xoff_correct_ = _xoff_correct_;
	_operate_dlg._xdisplay_		= &_xdisplay; 
	_operate_dlg._xtransfer_	= &_xtransfer;

	_control_dlg._xcommand_		= &_xcommand;
	_adv_dlg._xacquisition_		= &_xacquisition;
	_adv_dlg._xpiecewise_correct_ = &_xpiecewise_correct;
	_adv_dlg._xtransfer_		= &_xtransfer;
...
void CXEthernetDemoDlg::OnPaint()
{
	if (IsIconic())
	{
...
HCURSOR CXEthernetDemoDlg::OnQueryDragIcon()
{
...
void CXEthernetDemoDlg::OnTcnSelchangeTab1(NMHDR *pNMHDR, LRESULT *pResult)
{
...
public:
    ...
    void OnFrameReady(XImage* image_)
	{
		...
		{
			if(demo_dlg_->_en_offboard_correct)
				demo_dlg_->_xoff_correct_->DoCorrect(image_);
			else if(demo_dlg_->_en_piecewise_correct)
				demo_dlg_->_xpiecewise_correct.DoCorrect(image_);
		...
		else if(demo_dlg_->_is_non_hilo)
		{
			if(demo_dlg_->_en_offboard_correct)
				demo_dlg_->_xoff_correct_->DoCorrect(image_);
			//For hilo trigger mode, it combines two lines of different energy into one line. The high and low trigger should come symmetrically.
			demo_dlg_->_xdual_correct.DoCorrect(image_);
			demo_dlg_->_cur_image_ = demo_dlg_->_xdual_correct.GetImage();
			if(demo_dlg_->_cur_image_ )
			{
				//if(demo_dlg_->_en_offboard_correct)
				//	demo_dlg_->_xoff_correct.DoCorrect(demo_dlg_->_cur_image_);
				PostMessage(demo_dlg_->m_hWnd,WM_FRAME_READY,(WPARAM)demo_dlg_->_cur_image_,0);
				
				//Calculate average
				demo_dlg_->_xanalyze.DoAnalyze(demo_dlg_->_cur_image_, 2);
				//Calculate noise
				demo_dlg_->_xanalyze.DoAnalyze(demo_dlg_->_cur_image_,3);

				PostMessage(demo_dlg_->m_hWnd, WM_ANALYSIS, demo_dlg_->_cur_image_->_width,0);
			}
		}
		else if(demo_dlg_->_is_dual_line_lcs)
		{
			//For dual line LCS system
			if(demo_dlg_->_en_offboard_correct)
				demo_dlg_->_xoff_correct_->DoCorrect(image_);
			demo_dlg_->_xdual_line_correct.DoCorrect(image_);
			demo_dlg_->_cur_image_ = demo_dlg_->_xdual_line_correct.GetImage();
			if(demo_dlg_->_cur_image_ )
			{
				//if(demo_dlg_->_en_offboard_correct)
				//	demo_dlg_->_xoff_correct.DoCorrect(demo_dlg_->_cur_image_);
...
void CXEthernetDemoDlg::RegisterSink()
{
...
LRESULT	CXEthernetDemoDlg::OnFrameReadyMsg(WPARAM wParam, LPARAM lParam)
{
	_frame_counter++;
	CString str;
...
LRESULT CXEthernetDemoDlg::OnErrorMsg(WPARAM wParam, LPARAM lParam)
{
...
LRESULT CXEthernetDemoDlg::OnEventMsg(WPARAM wParam, LPARAM lParam)
{
	CString str;
	switch(wParam)
	{
...
LRESULT	CXEthernetDemoDlg::OnAnalysisMsg(WPARAM wParam, LPARAM lParam)
{
...
void CXEthernetDemoDlg::DrawMap(uint32_t* data_, uint32_t size)
{
	if(NULL == data_)
		return;
	CRect rect;
...
void CXEthernetDemoDlg::DrawMap(float* data_, uint32_t size)
{
	if(NULL == data_)
		return;
	CRect rect;
...
CWnd* CXEthernetDemoDlg::GetDispWnd()
{
	return GetDlgItem(IDC_DISPLAY_WND);
}
void CXEthernetDemoDlg::SaveImage(CString file_name)
{
	if(_cur_image_)
		_cur_image_->Save(file_name.GetBuffer());
}
void CXEthernetDemoDlg::OnClose()
{
	if(_xoff_correct_)
		_xoff_correct_->Close();
	_xpiecewise_correct.Close();
	_xdisplay.Close();
	_xacquisition.Close();
	_xcommand.Close();
	_xsystem.Close();
