...
void CXOperateDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Check(pDX, IDC_CHECK_EN_GAIN_OFFSET_OFFBOARD, _en_gf_correct_offboard);
}
...
BEGIN_MESSAGE_MAP(CXOperateDlg, CPropertyPage)
	...
	ON_BN_CLICKED(IDC_CHECK_EN_GAIN_OFFSET_OFFBOARD, &CXOperateDlg::OnBnClickedCheckEnGainOffsetOffboard)
END_MESSAGE_MAP()
...
void CXOperateDlg::OnBnClickedCheckEnGainOffset()
{
	...
	if(_en_gf_correct)
		GetDlgItem(IDC_CHECK_EN_GAIN_OFFSET_OFFBOARD)->EnableWindow(0);
	else
		GetDlgItem(IDC_CHECK_EN_GAIN_OFFSET_OFFBOARD)->EnableWindow(1);
}

void CXOperateDlg::OnBnClickedCheckEnGainOffsetOffboard()
{
	UpdateData();
	demo_dlg_->_en_offboard_correct = _en_gf_correct_offboard;
	if(_en_gf_correct_offboard)
		GetDlgItem(IDC_CHECK_EN_GAIN_OFFSET)->EnableWindow(0);
	else
		GetDlgItem(IDC_CHECK_EN_GAIN_OFFSET)->EnableWindow(1);
}
